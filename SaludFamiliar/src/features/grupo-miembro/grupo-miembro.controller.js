import * as useCases from './grupo-miembro.use-cases.js';
import { created, ok, noContent, notFound, badRequest, handleError } from '../../shared/http.js';

export async function create(req, res) {
  try {
    const { periodo_id, grupo_id, estudiante_id } = req.body;
    if (!periodo_id || !grupo_id || !estudiante_id) {
      return badRequest(res, 'periodo_id, grupo_id y estudiante_id son requeridos');
    }
...
export async function list(req, res) {
  try {
    const { periodo_id, grupo_id, estudiante_id } = req.query;
    const page = Math.max(parseInt(req.query.page) || 1, 1);
    const limit = Math.min(Math.max(parseInt(req.query.limit) || 20, 1), 100);
    return ok(res, await useCases.list({ periodo_id, grupo_id, estudiante_id, page, limit }));
  } catch (err) { return handleError(res, err); }
}
...
export async function remove(req, res) {
  try {
    if (!await useCases.remove(req.params.id)) return notFound(res, 'Miembro no encontrado');
